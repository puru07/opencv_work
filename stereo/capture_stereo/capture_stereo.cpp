...
int main(int, char**)
{
    ...
    CvCapture* capRight = cvCaptureFromCAM(1); // open the Right camera
    ...
    string leftname = "img_data/left";
    string rightname = "img_data/right";
    string filetype = ".jpg";
    ...
    for(;;)
    {
        ...
        //cv::Mat frameRight;
          ...
          IplImage* frameRight = cvQueryFrame(capRight); 
          IplImage* frameLeft = cvQueryFrame(capLeft); 
          cvShowImage("Left", frameLeft); 
          cvShowImage("Right", frameRight);
          ...
          //capRight >> frameRight; //get a new frame from right camera
            ...
            {            
                ...
                // saving left capture
                ostringstream oss;
                oss << leftname << i << filetype;
                string file = oss.str();
                const char * cfile = file.c_str();
                oss.str("");
                cvSaveImage(cfile, frameLeft);

                // saving right capture
                oss << rightname << i << filetype;
                file = oss.str();
                cfile = file.c_str();
                oss.str("");
                cvSaveImage(cfile, frameRight);                
            }
    ...
    cvReleaseCapture(&capLeft); //Release capture.
    cvReleaseCapture(&capRight); //Release capture.
    cvDestroyWindow("Left"); //Destroy Window
